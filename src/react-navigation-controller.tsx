...
class NavigationController extends React.Component<NavigationControllerProps, NavigationControllerState> {
    static Transition = Transition;
    private __isTransitioning: boolean;
    private __viewStates: number[];
    private __viewIndexes: [number, number];
    private __springSystem: rebound.SpringSystem;
    private __spring: rebound.Spring;
    private __nextViewList: any[];
    constructor(props) {
        super(props);
    ...
    __transitionViews(options) {
        ...
        // const prevView = this.refs[views.length - 2]
        ...
        // const nextView = this.refs[views.length - 1]
    ...
    __renderNode(node: React.ReactElement<any>, index): React.ReactElement<any> {
        if (!node) return null;
    ...
    render() {
        ...
        return (
            <div className={className} style={this.props.style}>
                <div ref={p => (this.references['view-wrapper--1'] = p)} />
                {this.state.views.map((item, index) => {
                    return (
                        <div key={index} ref={p => (this.references[`view-wrapper-${index}`] = p)} className={wrapperClassName}>
